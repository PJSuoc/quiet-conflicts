...
def small_big_split(df):
    ...
    big = pd.concat([ukr, pal, rus, isr, leb])
    big["conflict_size"] = "Palestine/Ukraine"
...
def map_merger(df):
    ...
    populations = pd.read_csv("../data/quickdata.csv", encoding = "ISO-8859-1")
    add2 = countries.merge(populations, 'left',left_on='name',right_on='Country')
    add2['pop'] = add2['Population'].fillna(0)

    #Migration ###s insert
    migration = pd.read_csv("../data/quickdata2.csv", encoding = "ISO-8859-1")
